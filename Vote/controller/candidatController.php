<?php
   ...
   if(isset($_POST['btn_valider_candidat'])){
    ...
    $photo=$_FILES["photo_parti"]["name"];
    //$upload="candidats/".$photo;
    move_uploaded_file($_FILES["photo_parti"]["tmp_name"], "candidats/".$_FILES['photo_parti']['name']);
    $ob_candidat=new Candidat();
    if($ob_candidat->addCandidat($nom_candidat,$nom_partie,$photo)){
...
//   modifier
   if(isset($_POST['btn_modif_candidat'])){
    ...
    echo $nom_partie;
    $ob_candidat=new Candidat();
    if($ob_candidat->MofifCandidat($id_candidat,$nom_candidat,$nom_partie,$photo)){
